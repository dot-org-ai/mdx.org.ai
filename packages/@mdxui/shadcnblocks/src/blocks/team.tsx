/**
 * Team Blocks (GAP: Not in @mdxui/html)
 */

import * as React from 'react'
import type { TeamBlockProps } from '../types'

export function TeamGrid({ headline, description, members }: TeamBlockProps) {
  return (
    <section aria-label="Team" className="py-20">
        ...
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          {members.map((member, i) => (
            <div key={i} className="text-center">
              {member.avatar && (
                <img src={member.avatar} alt={member.name} className="mx-auto mb-4 h-32 w-32 rounded-full object-cover" />
              )}
...
export const Team = { Grid: TeamGrid }

/**
 * Footer Blocks (GAP: Only container in @mdxui/html)
 */

import * as React from 'react'
import type { FooterBlockProps } from '../types'

export function FooterColumns({ logo, brand, columns, social, legal }: FooterBlockProps) {
  return (
      ...
      <div className="container mx-auto px-4">
        ...
        {legal && (
          <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t pt-8 md:flex-row">
            {legal.copyright && <p className="text-sm text-muted-foreground">{legal.copyright}</p>}
            {legal.links && (
              <div className="flex gap-6">
                {legal.links.map((link, i) => (
                  <a key={i} href={link.href} className="text-sm text-muted-foreground hover:text-foreground">
                    {link.label}
                  </a>
                ))}
              </div>

/**
 * `./format.ts` — content negotiation on the HTTP face (mdx-8je.8 + mdx-8je.18).
 *
 ...
 *
 * The `md` register is npm `@mdxui/text` (mdx-8je.8): nothing here reads a workspace @mdxui source.
 * NOTE: `@mdxui/text/core` `render(doc, 'md')` (this face, and the CLI's `--format md`) and
 * `@mdxui/text/md` `render(doc)` (the received `@mdxui/markdown`, used by @mdxe/workers' `.md` assets)
 * are two renderers in @mdxui/text@0.1.0 with different bytes: core emits the frontmatter as a
 * `## Type` data block and keeps JSX; `/md` keeps YAML frontmatter and strips JSX. The HTTP face
 * follows core so a served page is byte-identical to the CLI; the gap is tracked against dot-do/ui.
 */
...
This is **bold** and *italic* with {props.expr} inside.
...
describe('parseFormat — extensions', () => {
  ...
  it('maps nested paths and strips the extension', () => {
...
describe('renderDocument — the text registers render through @mdxui/text', () => {
  ...
  it('md renders through npm @mdxui/text (mdx-8je.8): document data kept, markdown body kept', () => {
    const { content } = renderDocument(doc, 'md')
    expect(content).toContain('title: Hello')
    expect(content).toContain('# Hello')
    expect(content).toContain('**bold**')
    expect(content).toContain('- one')
    // JSX / inline `{expr}` handling is the register's own contract in @mdxui/text@0.1.0 (core's md
    // register keeps them; `/md` strips flow JSX only) — tracked as a gap against dot-do/ui, not pinned here.
  })

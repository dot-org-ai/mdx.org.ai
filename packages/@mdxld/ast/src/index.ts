/**
 * @mdxld/ast - AST manipulation for MDXLD documents
 *
 * Provides functions for converting MDXLD documents to/from AST representation:
 * - toAst: Convert document to AST
 * - fromAst: Convert AST back to document
 * - parseWithAst: Parse content and include AST
 * - stringifyAst: Convert AST to string
 *
 * @packageDocumentation
 */

import type { MDXLDDocument, ParseOptions } from 'mdxld'
import { parse as parseDocument, stringify as stringifyDocument } from 'mdxld'

/**
 * AST node types supported by MDXLD
 */
...
/**
 * Parse MDX content into AST nodes
 */
 ...
 *
 * const doc = parseWithAst('# Hello World')
 * console.log(doc.ast) // AST representation
 ...
 * @example
 * ```ts
 * import { stringifyAst } from '@mdxld/ast'
 *
 * const ast = { type: 'root', children: [...] }
...
/**
 * Walk through AST nodes with a visitor function
 *
 ...
 * ```
 */
export function walk(ast: MDXLDAst | MDXLDAstNode, visitor: (node: MDXLDAstNode, parent?: MDXLDAstNode) => void): void {
  function walkNode(node: MDXLDAstNode, parent?: MDXLDAstNode): void {
    visitor(node, parent)
    if (node.children) {
  ...
  if ('type' in ast && ast.type === 'root') {
    for (const child of (ast as MDXLDAst).children) {
      walkNode(child, ast as MDXLDAstNode)
    }
  } else {
    walkNode(ast as MDXLDAstNode)
  }
}
 ...
 * ```
 */
export function findAll(ast: MDXLDAst | MDXLDAstNode, predicate: (node: MDXLDAstNode) => boolean): MDXLDAstNode[] {
  const results: MDXLDAstNode[] = []
 ...
 */
export function find(ast: MDXLDAst | MDXLDAstNode, predicate: (node: MDXLDAstNode) => boolean): MDXLDAstNode | undefined {
  let result: MDXLDAstNode | undefined
 ...
 *
 * @param ast - AST to search
 * @returns Array of heading nodes with text
 */
export function extractHeadings(ast: MDXLDAst): Array<{ depth: number; text: string; node: MDXLDAstNode }> {
  return findAll(ast, (node) => node.type === 'heading').map((node) => ({
    depth: node.depth as number,
    text: childrenToString(node.children || []),
    node,
  }))
}
 ...
 */
export function extractLinks(ast: MDXLDAst): Array<{ url: string; text: string; node: MDXLDAstNode }> {
  return findAll(ast, (node) => node.type === 'link').map((node) => ({
    url: node.url || '',
    text: childrenToString(node.children || []),
    node,
  }))
 ...
 */
export function extractCodeBlocks(ast: MDXLDAst): Array<{ lang?: string; value: string; meta?: string; node: MDXLDAstNode }> {
  return findAll(ast, (node) => node.type === 'code').map((node) => ({
    lang: node.lang,
    value: node.value || '',
    meta: node.meta,
    node,

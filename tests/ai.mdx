...
// Use unique type name to avoid test pollution
const testType = 'CrudTestItems'

// CREATE
await setTool.handler({
  url: 'https://test.example.com/' + testType + '/first',
  data: { title: 'First Item', tags: ['intro'] }
})

await setTool.handler({
  url: 'https://test.example.com/' + testType + '/second',
  data: { title: 'Second Item', tags: ['advanced'] }
})

// LIST
const listResult = await listTool.handler({ type: testType })
const listData = JSON.parse(listResult.content[0].text)
expect(listData.length).toBe(2)

// READ
const getResult = await getTool.handler({ id: testType + '/first' })
const getDoc = JSON.parse(getResult.content[0].text)
expect(getDoc.data.title).toBe('First Item')

// SEARCH - search within our specific type
const searchResult = await searchTool.handler({ query: 'Item', type: testType })
...
await setTool.handler({
  url: 'https://test.example.com/' + testType + '/first',
  data: { title: 'First Item (Updated)', tags: ['intro', 'updated'] }
})

const updatedResult = await getTool.handler({ id: testType + '/first' })
const updatedDoc = JSON.parse(updatedResult.content[0].text)
expect(updatedDoc.data.title).toBe('First Item (Updated)')

// DELETE
await deleteTool.handler({ id: testType + '/second' })

const finalList = await listTool.handler({ type: testType })

...
The `mdxdb` root package is the schema-first `DB()` facade that picks an
adapter from `DATABASE_URL`. Install it too if you want typed entities and
relationships instead of the document-level API shown below:
...
`get()` resolves to `null` for a missing document; it does not throw. Writes
that violate an option (for example `set()` on an existing path without
`overwrite`) reject with a plain `Error`:
...
The schema-first `DB()` facade in `mdxdb` also returns `null` from `get()`,
and throws typed errors from writes:

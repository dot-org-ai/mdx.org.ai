...
**UI Components** provides component templates and rendering packages for building interfaces from MDX documents. JSX-in-Markdown is [MDX](https://mdxjs.com)'s contribution; what MDXLD adds is that a component can be selected and typed from the document's `$type`.
...
Presentation adds **styling** on top of semantic output:
